...
- Installs are **not** placed in the default per-user dotnet directory. Instead they go into a shared, custom cache directory: `~/.cache/jenkins-pipeline/dotnet` on macOS/Linux, and `%LOCALAPPDATA%\cache\jenkins-pipeline\dotnet` on Windows (a collision-safe, one-level-deeper path than initially shipped — see `design.md` Decision 4). This keeps installed SDKs cacheable/reusable across jobs on the same agent independent of the default dotnet CLI location.
