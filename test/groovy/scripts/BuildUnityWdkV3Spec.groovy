...
class BuildUnityWdkV3Spec extends DeclarativeJenkinsSpec {
    private static final String SCRIPT_PATH = "vars/buildUnityWdkV3.groovy"

    @Unroll("publishes UPM or Paket WDK depending on versions #packageType")
    def "publishes UPM or Paket WDK depending on versions #packageType"() {
        ...
        def packageSetupStash = "${packageType}_setup_w" as String

        when: "running buildWDKAutoSwitch pipeline"
        println packageSetupStash
        inSandbox { buildWDK(unityVersions: [[version :"2019", packageType: packageType]], clearWs: true) }

        then: "stashes #packageType setup"
        stash.containsKey(packageSetupStash)
        and: "don't stashes other paket setup"
        !stash.any { it.key != packageSetupStash && it.key.endsWith("_setup_w") }
